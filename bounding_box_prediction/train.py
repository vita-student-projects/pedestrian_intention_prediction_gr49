...
def train_2d(args, net, train, val):
    ...
    for epoch in range(args.n_epochs):
        ...
        avg_pre = 0
        mAP = 0
        train_intent_targets = []
        train_intent_preds = []
        train_intent_accl = []
        counter = 0
        for idx, (obs_s, target_s, obs_p, target_p, target_c, label_c, behaviour, crossing_obs, files) in enumerate(train):
            counter += 1
            ...
            net.zero_grad()
            speed_preds, crossing_preds = net(speed=obs_s, pos=obs_p, bh = behaviour)
            ...
            train_intent_targets.extend(label_c)
            train_intent_preds.extend(intentions)
            train_intent_acc_loop = accuracy_score(train_intent_targets, train_intent_preds)
            train_intent_accl.append(train_intent_acc_loop)
        ...
        for idx, (obs_s, target_s, obs_p, target_p, target_c, label_c, behaviour, crossing_obs, files) in enumerate(val):
            counter+=1
            obs_s    = obs_s.to(device='cuda')
            target_s = target_s.to(device='cuda')
            obs_p    = obs_p.to(device='cuda')
            ...
            with torch.no_grad():
                ...
                val_intent_targets.extend(label_c)
                val_intent_preds.extend(intentions)
                val_intent_acc_loop = accuracy_score(val_intent_targets, val_intent_preds)
                val_intent_accl.append(val_intent_acc_loop)
        ...
        train_intent_acc = train_intent_accl
        val_intent_acc = val_intent_accl
        ...
        data.append([epoch, avg_epoch_train_s_loss, avg_epoch_val_s_loss, \
                    ...
                    ade, fde, aiou, fiou, intent_acc, train_intent_acc, val_intent_acc])
    ...
    df = pd.DataFrame(data, columns =['epoch', 'train_loss_s', 'val_loss_s', 'train_loss_c', 'val_loss_c',\
                'ade', 'fde', 'aiou', 'fiou', 'intention_acc', 'train_intention_acc', 'val_intent_acc']) 

...
class PV_LSTM(nn.Module):
    def __init__(self, args):
        ...
        ### Behaviour encoding ###
        self.bh_encoder = nn.LSTM(input_size=self.size, hidden_size=args.hidden_size)
    ...
    def forward(self, speed=None, pos=None, bh = None, average=False):
        ...
        if (np.shape(bh) == np.shape(speed)):
            _, (bhsp, bcsp) = self.bh_encoder(bh.permute(1,0,2))
            bhsp = bhsp.squeeze(0)
            bcsp = bcsp.squeeze(0)
        else:
            bhsp = 0

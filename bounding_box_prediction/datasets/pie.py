...
class PIE(torch.utils.data.Dataset):
    def __init__(self, 
        ...
        if(from_file):
        ...
        elif(load == 1): # if the data is already present as csv
            df = pd.read_csv(os.path.join(self.out_dir, self.filename))
            sequence_centric = df.copy()
            ...
            ac = df.groupby(['ID'])['action'].apply(list).reset_index(name='action').drop(columns='ID')
            ge = df.groupby(['ID'])['gesture'].apply(list).reset_index(name='gesture').drop(columns='ID')
            lo = df.groupby(['ID'])['look'].apply(list).reset_index(name='look').drop(columns='ID')
            nl = df.groupby(['ID'])['num_lanes'].apply(list).reset_index(name='num_lanes').drop(columns='ID')

            d = bb.join(s).join(f).join(c).join(ac).join(ge).join(lo).join(nl)
            ...
            action_o = np.empty((0,1))
            gesture_o = np.empty((0,1))
            look_o = np.empty((0,1))
            num_lanes_o = np.empty((0,1))
            ...
            for i in range(d.shape[0]):
                ...
                while (k+input+output) <= len(ped.bounding_box):
                    ...
                    action_o = np.vstack((action_o, np.array(ped.action[k:k+input]).reshape(input, 1)))
                    gesture_o =  np.vstack((gesture_o, np.array(ped.gesture[k:k+input]).reshape(input, 1)))
                    look_o =  np.vstack((look_o, np.array(ped.look[k:k+input]).reshape(input,1)))
                    num_lanes_o =  np.vstack((num_lanes_o, np.array(ped.num_lanes[k:k+input]).reshape(input, 1)))

                    k += stride
            
            behaviour = np.concatenate((action_o, gesture_o, look_o, num_lanes_o), axis = 1).reshape((-1,output, 4))
    ...
    def __getitem__(self, index):
        ...
        behaviour = torch.tensor([seq.behaviour[i] for i in range(0,self.input,self.skip)])
        outputs.append(behaviour.type(torch.float32))

        crossing_obs = torch.tensor([seq.crossing_obs[i] for i in range(0,self.input,self.skip)])
        filename = [seq.filename[0]] #for i in range(0,self.input,self.skip)]
        outputs.append(crossing_obs.type(torch.float32))
